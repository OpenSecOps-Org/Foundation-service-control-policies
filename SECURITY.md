# Security Policy — Foundation-service-control-policies
...
This component was converted to the locked-dependency model in **v1.0.9**. The supply-chain posture below applies to v1.0.9 and every release after it. Earlier releases predate the conversion and do not carry these guarantees; per §1, the remediation for any exposure on an earlier release is to upgrade to the current release.
...
```
https://github.com/OpenSecOps-Org/Foundation-service-control-policies/releases/download/<VERSION>/Foundation-service-control-policies-<VERSION>-sbom.cdx.json
```

Replace `<VERSION>` with the release tag (e.g. `v1.0.9`). The asset is attached to every release on the public OpenSecOps-Org remote.
...
Together they are the authoritative cross-component references; the per-component sections above describe how the project-wide posture applies to **Foundation-service-control-policies** specifically.
